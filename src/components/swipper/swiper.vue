<template>
  <div id="swiper" @mouseover="stopAuto" @mouseout="autoPlay">
    <div
      ...
      :style="{
        width: allCount,
        '-webkit-transition': transitionConfig,
        '-webkit-transform': slateX,
      }"
      ref="swiper"
    >
      <swiper-item
        :img="imgList[imgList.length - 1].img"
        :imgWidth="-imgWidth"
        v-if="loop"
      />
      <swiper-item
  ...
  methods: {
    toLeft() {
      if (this.loop) {
        ...
        if (this.imgIndex == 0)
          return (this.dotsIndex = this.imgIndex = this.imgList.length - 1);
        this.imgIndex--;
        this.dotsIndex--;
      }
    },
